...
[in English](README.md) · [Установка](docs/install.ru.md) · [Витрина](docs/gallery.ru.md) · [Документация](#документация) · [Участие](CONTRIBUTING.ru.md)

**Windows 10/11, x64.** Linux планируется и пока не сделан; в
`docs/install.ru.md` сказано, [что именно мешает](docs/install.ru.md#linux), а
...
Ещё четырнадцать кадров с разбором, что в каждом показано, —
в [docs/gallery.ru.md](docs/gallery.ru.md). Все сделаны сценами из этого
репозитория и ничем не ретушированы.
...
| [Редактор](docs/forge.md) | forge: воксельные модели и пиксельные текстуры, управление, экспорт |
| [Установка](docs/install.ru.md) | Что поставить, как собрать и что делать, когда не собирается |
| [Тесты](docs/testing.md) | Что и как покрыто |
| [Витрина](docs/gallery.ru.md) | Кадры и что в каждом видно |
...
находятся в PATH**. `build.cmd` сам находит установку — через `vswhere` или
через `BLOCKY_VSROOT`, если хотите сказать прямо, — поднимает окружение x64 и
делает всё остальное; вызывать `cmake` напрямую не нужно.

По шагам, вместе с тем, какие компоненты отметить и что означают ошибки, —
...
**Требуется:** MSVC 14.5+ (C++20), Windows SDK и компонент «Средства CMake для
Windows», откуда берутся CMake и Ninja. Больше ничего — докачивать движку
нечего. Игровые ассеты Minecraft **не обязательны**: без них сцены рендерятся
плоской палитрой, а игра генерирует свои текстуры.
...
Все временные замеры, которые встречаются в этом репозитории, сделаны на одной
машине, и это ноутбук:

| | |
|---|---|
| Процессор | Intel Core i7-13620H — 10 ядер, 16 потоков, базовая 2.4 ГГц |
| Память | 16 ГБ, 5600 MT/s |
| Видеокарта | NVIDIA GeForce RTX 4050 Laptop, 6 ГБ, драйвер 596.36, OpenGL 4.6 |
| | Рядом Intel UHD Graphics — переключаемая графика, работать надо на дискретной |
| Диск | NVMe SSD |
| ОС | Windows 11, сборка 26200 |
| Тулчейн | MSVC 14.50.35717, Windows SDK 10.0.26100, CMake 4.2.3, Ninja 1.12.1 |

**Она троттлит, и это важнее номеров моделей.** Одна и та же однопоточная
работа занимала на ней от 3.8 до 9.8 секунды в зависимости только от того,
насколько машина успела прогреться, — примерно втрое, при неизменном всём
остальном. Поэтому каждое число здесь — это **отношение, измеренное внутри
одного прогона**, а не показание секундомера, которое стоит сравнивать со
своим; там, где это существенно, так и написано рядом с числом.

Требованием это не является. Процессорному рендереру нужен только компилятор, а
вьюпорту — OpenGL 4.6, см. [docs/install.ru.md](docs/install.ru.md#видеокарта).
Раздел нужен затем, чтобы «3.4 секунды против 68» что-то значило.
...
engine/edit/            документы редактора: отмена, пиксельный холст,
                        воксельная лепка, россыпь спрайтов; выгрузка в исходник
                        на C++, в лист слоёв и в модель Minecraft
...
plugins/<имя>/          всё вместе → один <имя>.exe. Сейчас это forge
```

Движок — 35 тысяч строк в 189 файлах, сцены — 21.5 тысячи в 55, игра — 6.5
тысячи в тридцати одном, редактор — 1.8 тысячи в двенадцати.
...
Третий вход, и третья форма сборки: всё, что лежит в `plugins/<имя>/`,
собирается в `<имя>.exe`. Плагин здесь — **программа на движке**, а не
библиотека внутрь него: ничего в этом проекте не загружается в рантайме, и
чтобы это стало возможно, понадобились бы стабильный C-ABI, реестр и история
версий.

Три документа в одном окне, `Tab` по кругу.

| | |
|---|---|
| **Модель** | Воксельная сетка. Левая кнопка ставит, `Shift` стирает, правая крутит камеру. Наведение — тот же `trace`, что даёт игре «сломать/поставить», только против сетки, а не решётки |
| **Холст** | Пиксельная картинка. 16×16 — текстура блока, она же предмет, который выдавит `item::buildModel`; 64×64 — скин, и над ним рисуются подсказки раскладки, взятые у `Skin::faceRect`, а не переписанные рядом |
| **Спрайты** | Квады в мире: пылинки, искры, парящий текст. Разворот к камере замораживается при постановке — это позиция движка, а не упрощение инструмента |

**Всё выходит исходником на C++**, как сцены, как карты, как палитры: модель —
вызовом `voxelize::fromLayers`, холст — вызовом `pixelart::fromRows`, спрайты —
функцией, возвращающей `std::vector<Sprite>`. Текстура, написанная так,
перестаёт быть файлом ассета и становится кодом — тем же, чем уже являются
генерируемые текстуры блоков.

**А `P` пишет ресурспак для Minecraft.** Ванильная модель: `elements` из
кубоидов плюс палитровая текстура, в дереве `assets/<ns>/models/...`. Формат у
мода и у пака один, так что один выход служит обоим. Кубоиды берёт то же самое
точное жадное слияние, которым коллайдер разбивает модель на коробки, — только
с отношением «того же материала» вместо «сплошной».

Формата у моделей здесь по-прежнему нет, и это то же решение, по которому карты
— код. Подробности, управление и то, чего в редакторе нет, — в
...
- Текстур у спрайтов во вьюпорте: `buildSpriteMesh` появился вместе с
  редактором и рисует квад заливкой его тинта, чего хватает, чтобы видеть, где
  спрайт и какого он размера. Картинку на нём по-прежнему показывает только
  трассировка. Пропы и сущности вьюпорт рисует с 2026-09-02, причём сущности
  перемешиваются каждый кадр — позу дешевле пересобрать, чем спросить,
  менялась ли она.
...
Пул-реквесты приветствуются, а [CONTRIBUTING.ru.md](CONTRIBUTING.ru.md)
говорит, какие — нет, чтобы никто не потратил вечер впустую. Две вещи стоит
знать до всего остального: **сторонние библиотеки не принимаются** ни в одном
из обличий, потому что писать их — это и есть данный проект; и **ИИ
приветствуется, а ответственность непередаваема** — если не можете защитить
строку на ревью, не отправляйте её.

---

## Лицензия

MIT — [LICENSE](LICENSE). Делайте что хотите, сохраните копирайт, гарантий
нет. В [NOTICE](NOTICE) лежит то, чего лицензия не говорит: оговорка о
товарном знаке, откуда взялись кадры витрины и тот факт, что стороннего кода
здесь нет и указывать в атрибуции нечего.

Движок не связан с Mojang Studios и Microsoft; «Minecraft» — их товарный знак.
Игровых ассетов здесь нет и не требуется ни одного: текстуры блоков строятся
кодом, персонажи рисуются кодом, шрифт свой. Что движок умеет — это **читать**
уже установленную у вас игру, и часть кадров в [витрине](docs/gallery.ru.md)
снята именно так; они помечены, и рисунок в них принадлежит своим владельцам.
