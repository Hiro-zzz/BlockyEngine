...
Движок — 32 тысячи строк в 171 файле, сцены — 20 тысяч в 50, игра — 5.6 тысячи
в двадцати пяти.
...
- Копирования и удаления инструментом: физган берёт то, что уже есть.
  Дублирование — это `PhysicsWorld::add` рядом с уже работающим `remove`, но
  решать, откуда берётся модель, отдельный вопрос.
